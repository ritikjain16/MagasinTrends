...
app.post("/updateaddress", (req, res) => {
  const { uid, addressitem, addressID } = req.body;
  Userlist.updateOne(
    { uid: uid, "address.address_id": addressID },
    {
      $set: {
        "address.$.address_id": addressID,
        "address.$.city": addressitem.city,
        "address.$.mobileno": addressitem.mobileno,
        "address.$.name": addressitem.name,
        "address.$.pincode": addressitem.pincode,
...
app.get("/getcarousellist", async (req, res) => {
  try {
    const carousels = await CarouselList.find();
